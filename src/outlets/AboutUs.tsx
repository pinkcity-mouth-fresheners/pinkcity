...
const AboutUs = () => {
  ...
  return (
    <div className="w-full py-14 px-0 flex flex-col justify-center items-center gap-6 relative">
      ...
      {/* About Us Content */}
      <div
        className={`${isMobile ? "flex-col" : "flex-row mt-14"
          } flex items-start justify-between w-full`}
      >
        <div
          className={`rounded-r-[50] max-w-1/3 overflow-hidden z-10 sticky relative ${isMobile ? "hidden" : "top-45"
            }`}
        >
          <Image
            src={AboutUsLogo}
        ...
        <div
          className={`${isMobile ? "w-full px-6" : "max-w-3/5 px-18"
        ...
        >
          {isMobile && (
            <div className="w-full flex justify-start place-items-stretch gap-3">
              ...
              <h2
                className={`font-semibold ${isMobile ? "text-xl" : "text-2xl py-2"
          ...
          {!isMobile && (
            <div>
              <h2
                className={`font-semibold ${isMobile ? "text-md" : "text-2xl py-2"
                  } sticky top-40 bg-white z-10`}
              >
          ...
          <div
            className={
              isMobile
                ? "w-[65%] mx-auto -my-6 rounded-2xl overflow-hidden"
                : "hidden"
            }
          >
            {/* <Image
      ...
      {/* Philosophy Section */}
      <div
        className={`w-full flex items-start justify-between ${isMobile ? "flex-col-reverse px-2" : "py-20 px-14"
          }`}
      >
        <div
          className={`${isMobile ? "w-full px-4 text-xs gap-4" : "max-w-2/5 text-lg  gap-12"
          ...
          {isMobile ? (
            <div className="relative w-2/3 mx-auto mb-8 px-4">
              {philosophyImages.map((src, index) => (
                <div
                  key={index}
                  className={`transition-opacity duration-700 ease-in-out ${index === currentSlide
                    ? "opacity-100 w-full"
